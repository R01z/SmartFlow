...
export class TeamsRegisterComponent {
  ...
  getTeam(teamId: string): void {
    this.http.get<any>(`${this.apiUrl}/api/v1/teams/getTeamById/` + teamId).subscribe(team => {
      this.teamData = team;
    });
  }
  ...
  submitForm(): void {
    ...
    this.http.post(`${this.apiUrl}/api/v1/teams/save`, this.teamData).subscribe(response => {
  ...
  deleteTeam(): void {
    ...
    this.http.delete(`${this.apiUrl}/api/v1/teams/deleteTeam/${teamId}`).subscribe(response => {
